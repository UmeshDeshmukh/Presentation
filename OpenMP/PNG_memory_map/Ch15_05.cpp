...
long unsigned int i;
void ConvertRgbToGsCpp_omp(uint8_t* pb_gs, const RGB32* pb_rgb, size_t num_pixels, const float* coef,int n_threads)
{   
...
void ConvertRgbToGs(void)
{
    ...
    const char* fn_gs1 = "./ConvertRgbToGs_ImageC_GS1.png";
    const char* fn_gs2 = "./ConvertRgbToGs_ImageC_GS2.png";
    const char* fn_gs3 = "./ConvertRgbToGs_ImageC_GS3.png";
	...
	if (CompareGsImages(pb_gs1, pb_gs3, num_pixels))
            {cout << "Grayscale image3 compare OK\n";}
        else
		{cout << "Grayscale image3 compare failed\n";}
    ...
    if (rc1 && rc2)
    {
		...
		cout << "Saving grayscale image #3 - " << fn_gs3 << nl;
        im_gs3.SaveToPngFile(fn_gs3);

        if (CompareGsImages(pb_gs1, pb_gs2, num_pixels))
            cout << "Grayscale image2 compare OK\n";
        else
            cout << "Grayscale image2 compare failed\n";
